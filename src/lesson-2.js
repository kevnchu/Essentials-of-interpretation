...
Lexer.prototype = {
    ...
    readNextToken: function () {
        ...
        if (/\d/.test(this.source.getCurrentChar()))
    ...
    readDigit: function () {
        var tokenValue = "";
        while (/\d/.test(this.source.getCurrentChar())) {
            // collect the digits into a number
            tokenValue += this.source.getCurrentChar();
            // and moving forward
            this.source.readNextChar();
        }
        return tokenValue;
    },
    ...
    readSymbol: function () {
        // get token value
        var tokenValue = this.source.getCurrentChar();
...
Lexer.tokenize = function (source) {
    ...
    } while (lexer.source.getCurrentChar());
    return tokens;
};
